...
namespace ItemProject.Trade.Forms.Container
{
    public partial class AddPlaceForm : Form
    {
        ...
        public AddPlaceForm(DatabaseInterface db, TradeStoreContainer Container_,TradeStorePlace place)
        {
            InitializeComponent();
            DB = db;
            placesql = new TradeStorePlaceSQL(DB);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
           ...
           if(Add.Name =="Update")
            {
                bool r = placesql.UpdatePlace   (_Container ,place .PlaceID, TextBoxName.Text,textBoxDesc .Text );
                if (r == true)
                {
                    this.DialogResult = DialogResult.OK; this.Dispose();
                }
